...
try {
    ...
    // Step 2: Fetch obstetrical history records with doctor name
    $sql = "
        SELECT 
            oh.*, 
            CONCAT('Dr. ', COALESCE(u.first_name, 'Unknown'), ' ', COALESCE(u.last_name, '')) AS examined_by
        FROM obstetrical_history oh
        LEFT JOIN users u ON oh.staff_id = u.id
        WHERE oh.patient_id = ?
    ";
    $params = [$patientId];

    if ($date) {
        $sql .= " AND DATE(oh.visit_date) = ?";
        $params[] = $date;
        error_log("Filtering obstetrical history by date: $date");
    }

    $sql .= " ORDER BY oh.visit_date DESC";
    ...
    echo json_encode([
        'status' => 'success',
        'data' => $records,
        'message' => count($records) ? null : 'No obstetrical history records found'
    ]);
