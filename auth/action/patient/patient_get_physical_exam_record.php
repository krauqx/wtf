...
try {
    ...
    // Step 2: Fetch physical exam records with doctor name
    $sql = "
        SELECT 
            per.*, 
            CONCAT('Dr. ', COALESCE(u.first_name, 'Unknown'), ' ', COALESCE(u.last_name, '')) AS examined_by
        FROM physical_examination_record per
        LEFT JOIN users u ON per.staff_id = u.id
        WHERE per.patient_id = ?
    ";
    $params = [$patientId];

    if ($date) {
        $sql .= " AND per.exam_date = ?";
        $params[] = $date;
        error_log("Filtering physical exams by date: $date");
    }

    $sql .= " ORDER BY per.exam_date DESC";
    ...
    echo json_encode([
        'status' => 'success',
        'data' => $records,
        'message' => count($records) ? null : 'No physical examination records found'
    ]);
