...
try {
    ...
    // Step 2: Fetch VAW risk records with doctor name
    $sql = "
        SELECT 
            vrr.*, 
            CONCAT('Dr. ', COALESCE(u.first_name, 'Unknown'), ' ', COALESCE(u.last_name, '')) AS examined_by
        FROM vaw_risk_assessment vrr
        LEFT JOIN users u ON vrr.staff_id = u.id
        WHERE vrr.patient_id = ?
    ";
    $params = [$patientId];

    if ($date) {
        $sql .= " AND DATE(vrr.visit_date) = ?";
        $params[] = $date;
        error_log("Filtering VAW risk records by date: $date");
    }

    $sql .= " ORDER BY vrr.visit_date DESC";
    ...
    echo json_encode([
        'status' => 'success',
        'data' => $records,
        'message' => count($records) ? null : 'No VAW risk records found'
    ]);
