...
session_start();
if (!isset($_SESSION['midwife_name'])) {
    $_SESSION['midwife_name'] = 'Midwife Gregorio';
}
...
<head>
    ...
    <link rel="stylesheet" href="mwdash.css">
    <!--
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="sidebar">
        ...
        <div class="midwife-info">
            <div class="midwife-avatar">👨‍⚕️</div>
            <div class="midwife-name"><?php echo $_SESSION['midwife_name']; ?></div>
        </div>
    ...
    <div class="main-content">
        <div class="content-section">
            <h2 style="margin-bottom: 28px; color: #222;">Welcome Midwife Gregorio</h2>
        ...
        <div id="services-section" class="content-section" style="margin-top: 25px; display: none; background: rgba(255, 255, 255, 0.95);">
            ...
            <div id="services-blank-state" style="text-align: center; padding: 80px 20px; color: #6b7280;">
                ...
                <!-- Add Service Button -->
                <button class="btn-primary" id="openAddServiceModalFromBlank" type="button"
                    onclick="console.log('Button clicked'); document.getElementById('addServiceModal').style.display='flex'; console.log('Modal display set to flex');"
                    style="padding: 16px 32px; font-size: 16px; font-weight: 600; border-radius: 12px; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); color: white; border: none; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 8px 24px rgba(102, 126, 234, 0.3);">
                    + Add New Service
                </button>
            ...
            <div id="services-content" style="display: none;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px;">
                    ...
                    <button class="btn-primary" id="openAddServiceModal" type="button"
                        onclick="console.log('Regular button clicked'); document.getElementById('addServiceModal').style.display='flex'; console.log('Modal display set to flex');"
                        style="padding: 16px 32px; font-size: 16px; font-weight: 600; border-radius: 12px; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); color: white; border: none; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 8px 24px rgba(102, 126, 234, 0.3);">
                        + Add New Service
                    </button>
                </div>
           ...
           <div id="vaw-risk-content" class="tab-content" style="display: none;">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 16px; padding-top: 24px; border-top: 1px solid #e2e8f0; flex-shrink: 0; margin-bottom: 0;">
              ...
              <button type="button" id="closePhysicalExamModal" style="padding: 14px 28px; border-radius: 12px; font-size: 15px; font-weight: 600; background: #f8fafc; color: #475569; border: 2px solid #e2e8f0; cursor: pointer; transition: all 0.3s ease; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; min-width: 120px;">Done</button>
                ...
                <!-- Add Service Modal -->
                <div id="addServiceModal" class="modal-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 9999; justify-content: center; align-items: center; flex-direction: column;">
    ...
    <script>
            ...
            console.log('DOM Content Loaded - Initializing Add Service Modal functionality');
            ...
            const openAddServiceModalFromBlank = document.getElementById('openAddServiceModalFromBlank');
            
            console.log('Elements found:', {
                openAddServiceModal: !!openAddServiceModal,
                addServiceModal: !!addServiceModal,
                closeAddServiceModal: !!closeAddServiceModal,
                cancelAddServiceModal: !!cancelAddServiceModal
            });

            // Central helpers to open/close the Services modal
            function openServicesModal() {
                if (!addServiceModal) return;
                // Move overlay to <body> to escape any stacking contexts in parents
                try {
                    if (addServiceModal.parentElement !== document.body) {
                        document.body.appendChild(addServiceModal);
                    }
                } catch (_) {}
                
                addServiceModal.classList.add('show');
                // Fallback inline style for older code paths
            ...
            if (openAddServiceModal) {
                console.log('Adding event listener to regular Add Service button');
                // Open modal
                openAddServiceModal.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    openServicesModal();
                });
            } else {
                console.error('Regular Add Service button not found!');
            }

            // Add event handler for the "Add New" button in blank state
            console.log('Blank state button found:', !!openAddServiceModalFromBlank);
            if (openAddServiceModalFromBlank) {
                console.log('Adding event listener to blank state button');
                openAddServiceModalFromBlank.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    openServicesModal();
                });
            } else {
                console.error('Blank state button not found!');
            }

            if (closeAddServiceModal) {
                // Close modal (X button)
                closeAddServiceModal.addEventListener('click', function() { closeServicesModal(); });
            }

            if (cancelAddServiceModal) {
                // Cancel modal (Cancel button)
                cancelAddServiceModal.addEventListener('click', function() { closeServicesModal(); });
            }
                    ...
                    serviceEntry.innerHTML = `
                        <div class="form-grid">
                            ...
                            <div class="form-group hospital-form-group">
                                ...
                                <div class="input-wrapper">
                                    <input type="number" id="service_amount_1" name="service_amount[]" class="hospital-input" min="0" step="0.01" placeholder="0.00" required>
                                </div>
                            </div>
                            ...
                            <div class="form-group hospital-form-group">
                                ...
                                <div class="input-wrapper">
                                    <input type="number" id="service_amount_${serviceCount}" name="service_amount[]" class="hospital-input" min="0" step="0.01" placeholder="0.00" required>
                                </div>
                            </div>
        ...
        // Removed temporary doctor search input overlay to declutter the modal
        ...
        // Debug: Test function to manually show Add Service Modal
        window.testShowAddServiceModal = function() {
            console.log('Testing Add Service Modal display...');
            const addServiceModal = document.getElementById('addServiceModal');
            if (addServiceModal) {
                addServiceModal.style.display = 'flex';
                console.log('Add Service Modal manually displayed');
                console.log('Modal element:', addServiceModal);
                console.log('Modal display style:', addServiceModal.style.display);
                console.log('Modal computed styles:', window.getComputedStyle(addServiceModal));
            } else {
                console.error('Add Service Modal not found in test function');
            }
        };
