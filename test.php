...
<body>
        ...
        <div id="emailVerificationModal" class="modal-overlay" style="display: none;">
            <div class="modal-content" style="max-width: 400px;">
                ...
                <div class="modal-form">
                    <p style="color: #64748b; margin-bottom: 20px;">We've sent a verification code to your email address. Please enter the code below to complete the password change.</p>
                    
                    <div class="form-group">
                        <label>Verification Code</label>
                        <input type="text" id="verificationCode" placeholder="Enter 6-digit code" maxlength="6" style="text-align: center; font-size: 18px; letter-spacing: 2px;">
                    </div>
                    
                    <div class="modal-actions">
                        <button type="button" class="btn-cancel" onclick="closeEmailVerificationModal()">Cancel</button>
                        <button type="button" class="btn-save" onclick="verifyEmailCode()">Verify & Change Password</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="medical-records-section" class="content-section" style="margin-top: 30px; display: none; background: #f9fafc; border-radius: 20px;">
            <div style="max-width: 1200px; margin: 0 auto; padding: 32px; display: grid; grid-template-columns: 1fr 2fr; gap: 32px; align-items: flex-start;">
                ...
                <div style="min-width: 320px; max-width: 420px; display: flex; flex-direction: column; gap: 24px;">
                    ...
                    <div style="height: 562px; background: white; border-radius: 20px; padding: 32px; box-shadow: 0 4px 24px rgba(108, 51, 110, 0.06); display: flex; flex-direction: column; gap: 32px;">
                        <div>
            ...
            <!-- Risk Assessment Card - Match System Design -->
            <div style="background: #fff; border-radius: 18px; box-shadow: 0 4px 24px rgba(44,62,80,0.08); padding: 24px 24px; margin-bottom: 24px; width: 100%;">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                    ...
                    <div id="riskLevelIndicator" style="padding: 8px 16px; border-radius: 20px; font-size: 14px; font-weight: 600; background: #e2e8f0; color: #64748b; border: 2px solid #cbd5e1; transition: all 0.3s ease;">
                        Not Set
                    </div>
                </div>
                ...
                <div style="margin-top: 20px;">
                    ...
                    <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                        <button type="button" id="notSetBtn" onclick="setRiskLevel('notset')" style="display: flex; align-items: center; gap: 8px; padding: 10px 16px; border-radius: 20px; border: 2px solid #cbd5e1; background: #e2e8f0; color: #64748b; cursor: pointer; transition: all 0.3s ease; font-size: 14px; font-weight: 600;" class="selected">
                            <span>⭕</span>
                            <span>Not Set</span>
                        </button>
                        <button type="button" id="lowRiskBtn" onclick="setRiskLevel('low')" style="display: flex; align-items: center; gap: 8px; padding: 10px 16px; border-radius: 20px; border: 2px solid #10b981; background: transparent; color: #10b981; cursor: pointer; transition: all 0.3s ease; font-size: 14px; font-weight: 600;">
                            <span>✅</span>
                            <span>Low Risk</span>
                        </button>
                        <button type="button" id="highRiskBtn" onclick="setRiskLevel('high')" style="display: flex; align-items: center; gap: 8px; padding: 10px 16px; border-radius: 20px; border: 2px solid #ef4444; background: transparent; color: #ef4444; cursor: pointer; transition: all 0.3s ease; font-size: 14px; font-weight: 600;">
                            <span>⚠️</span>
                            <span>High Risk</span>
                        </button>
                    </div>
                </div>
                <input type="hidden" id="patientRiskLevel" name="patient_risk_level" value="notset">
            </div>
    ...
    <script>
        ...
        // Risk Assessment Toggle Function
        function setRiskLevel(riskLevel) {
            const patientRiskLevel = document.getElementById('patientRiskLevel');
            const notSetBtn = document.getElementById('notSetBtn');
            ...
            patientRiskLevel.value = riskLevel;
            
            // Reset all buttons to unselected state
            notSetBtn.style.background = '#e2e8f0';
            notSetBtn.style.color = '#64748b';
            notSetBtn.style.border = '2px solid #cbd5e1';
            
            lowRiskBtn.style.background = 'transparent';
            lowRiskBtn.style.color = '#10b981';
            lowRiskBtn.style.border = '2px solid #10b981';
            
            highRiskBtn.style.background = 'transparent';
            highRiskBtn.style.color = '#ef4444';
            highRiskBtn.style.border = '2px solid #ef4444';
            
            // Reset indicator to default state
            riskLevelIndicator.style.background = '#e2e8f0';
            riskLevelIndicator.style.color = '#64748b';
            riskLevelIndicator.style.border = '2px solid #cbd5e1';
            
            // Set selected button styling and update indicator
            if (riskLevel === 'notset') {
                notSetBtn.style.background = '#cbd5e1';
                notSetBtn.style.color = '#475569';
                notSetBtn.style.border = '2px solid #94a3b8';
                
                riskLevelIndicator.textContent = 'Not Set';
                riskLevelIndicator.style.background = '#e2e8f0';
                riskLevelIndicator.style.color = '#64748b';
                riskLevelIndicator.style.border = '2px solid #cbd5e1';
            } else if (riskLevel === 'low') {
                lowRiskBtn.style.background = '#10b981';
                lowRiskBtn.style.color = '#ffffff';
                lowRiskBtn.style.border = '2px solid #10b981';
                
                riskLevelIndicator.textContent = 'Low Risk';
                riskLevelIndicator.style.background = '#dcfce7';
                riskLevelIndicator.style.color = '#166534';
                riskLevelIndicator.style.border = '2px solid #10b981';
            } else if (riskLevel === 'high') {
                highRiskBtn.style.background = '#ef4444';
                highRiskBtn.style.color = '#ffffff';
                highRiskBtn.style.border = '2px solid #ef4444';
                
                riskLevelIndicator.textContent = 'High Risk';
                riskLevelIndicator.style.background = '#fef2f2';
                riskLevelIndicator.style.color = '#991b1b';
                riskLevelIndicator.style.border = '2px solid #ef4444';
            }
            
            console.log('Risk level set to:', riskLevel);
        }

        // Initialize risk level on page load
        document.addEventListener('DOMContentLoaded', function() {
            // Check if there's a saved risk level in localStorage or from backend
            const savedRiskLevel = localStorage.getItem('patientRiskLevel');
            if (savedRiskLevel) {
                setRiskLevel(savedRiskLevel);
            } else {
                // Set default to "notset"
                setRiskLevel('notset');
            }
        });
        ...
        function saveProfileSettings() {
            ...
            console.log('Saving profile settings:', { name, email, phone, license });
        ...
        function showEmailVerificationModal() {
            ...
            // Generate and send verification code (in real app, this would be sent to server)
        ...
        function saveSystemPreferences() {
            ...
            console.log('Saving system preferences:', { emailNotifications, autoSave, darkMode });
        ...
        function showNotification(message, type = 'info') {
            ...
            if (type === 'success') {
                ...
                notification.style.background = 'linear-gradient(90deg, #667eea 0%, #764ba2 100%)';
            }
